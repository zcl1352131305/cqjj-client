        ...
        request(option, function (err, resp, body) {
            if (err) {
                ...
                if (data.code == "401") {
                    ...
                    //后台路径
                    if(null != data.data){
                        data.data.loginUserId = req.cookies.loginUserId;
                        data.loginUserId = req.cookies.loginUserId;
                    }
                    else{
                        data.loginUserId = req.cookies.loginUserId;
                    }

                    data.basePath = constant.REAL_PATH;
                    data.systemPath = constant.SYSTEM_INDEX;
                    resolve(data);
                }
            }
        });
